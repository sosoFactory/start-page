...
5. **북마크 JSON 내보내기 / 가져오기 (Export & Import)**:
   - 헤더 우측에 `[ ⬇ 내보내기 | ⬆ 가져오기 ]` 세그먼트 버튼 그룹(`.btn-group`) 배치.
   - **내보내기**: 현재 화면 순서 그대로 정제된 JSON 파일(`startpage-bookmarks-YYYYMMDD.json`) 다운로드.
   - **가져오기**: 파일 선택 시 스키마 유효성 검사(`id`, `title`, `url`) 및 확인 대화상자를 거쳐 안전하게 일괄 복원.
6. **실시간 인라인 검색 및 필터링 (Instant Search & Filter)**:
   - 별도의 검색 버튼 없이, 검색창에 키워드를 입력하는 즉시 사이트 이름(`title`) 또는 URL(`url`)을 대소문자 구분 없이 실시간 필터링.
   - 검색창은 카드 헤더의 액션 버튼 좌측에 깔끔한 검색 아이콘(`Search`)과 함께 미니멀 인풋 형태로 배치.
   - 검색어 입력 시 우측에 원클릭 지우기(`X`) 버튼 제공 및 `ESC` 키 입력 시 검색어 초기화 지원.
   - 검색 결과가 없을 경우 정갈한 "검색 결과가 없습니다" 안내 상태 표시.
   - 검색 필터링이 활성화된 상태에서는 드래그 앤 드롭 오동작을 방지하기 위해 정렬 잠금 처리.

### 3.2 ⭐ 크롬 확장 프로그램 통합 (Extension Integration)
